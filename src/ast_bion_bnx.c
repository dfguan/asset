...
//filter duplicates
diff_ary_t *col_diff_cov3(char *xmap, ctg_pos_t *rmap, qmap_ary_t *qmap, sdict_t *ctgs, float min_conf, int n_ctg)
{
	...
	char *algs = 0;
	int pref_id = -1, pqry_id = -1;	
	float pref_s = -1, pref_e = -1;	
	float preconf = 0;
	while (amap_read_x(fp, &r) >= 0)  {
		if (r.conf > min_conf) {
			if (r.ref_id != pref_id || r.ref_s != pref_s || r.ref_e != pref_e) {
				if (algs) {
					col_cors(algs, c);
					...
					for ( i = 0; i < c->n - 1; ++i) {
						...
						if (rs_ind == re_ind || qs_ind == qe_ind) continue; // this will remove the insertion and deletions, don't know if this is right, will be excluded for outlier analysis
						if (r.ori == '-') {
							int tmp = qs_ind;
							qs_ind = qe_ind;
							qe_ind = tmp;
						}		
						uint32_t s = rmap->ctg_pos[pref_id - 1].p[re_ind -1], e = rmap->ctg_pos[pref_id-1].p[rs_ind - 1];
						uint32_t diff_ref =  s - e;
						/*fprintf(stderr, "qry_id:%d\t%d\t%d\n",r.qry_id, qs_ind, qe_ind);		*/
						uint32_t diff_qry = qmap[pqry_id].q[qe_ind - 1].pos - qmap[pqry_id].q[qs_ind - 1].pos;
						int diff = diff_ref > diff_qry ? diff_ref - diff_qry : diff_qry - diff_ref;
						int ndiff = (float)diff / diff_ref * ctgs->seq[pref_id - 1].len;
						/*float cv = 0;*/
						/*int j;*/
						...
						diff_t diff_tmp = (diff_t) {ndiff, e, s};
						diff_ary_push(&diff_cov[pref_id - 1], &diff_tmp);
					}
				}
				pref_id = r.ref_id, pqry_id = r.qry_id, pref_s = r.ref_s, pref_e = r.ref_e, preconf = r.conf;
			} else if (r.conf > preconf) {
				if (algs) free(algs);
				algs = strdup(r.alignment);
				pqry_id = r.qry_id;	
				preconf = r.conf;
			}
		}
	...
	while (amap_read_x(fp, &r) >= 0)  {
		if (r.conf > min_conf) {
			...
			for ( i = 0; i < c->n - 1; ++i) {
				...
				/*fprintf(stderr, "qry_id:%d\t%d\t%d\tref_id:%d\t%d\t%d\t%d\n",r.qry_id, qs_ind, qe_ind, r.ref_id, rs_ind, re_ind, diff);	*/
				/*fprintf(stderr, "%s\t%d\t%d\t%d\t%d\t%d\t%d\n", ctgs->seq[r.ref_id-1].name, s, e, r.qry_id, qs_ind, qe_ind, diff_qry);	*/
				/*fprintf(stderr, "%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", ctgs->seq[r.ref_id-1].name, rs_ind, re_ind, e, s, r.qry_id, qs_ind, qe_ind, qmap[r.qry_id].q[qe_ind-1].pos, qmap[r.qry_id].q[qs_ind-1].pos, ndiff);	*/
				/*fprintf(stderr, "%s\t%d\t%d\t%d\n", ctgs->seq[r.ref_id-1].name, e, s,diff);	*/
				diff_t diff_tmp = (diff_t) {ndiff, e, s};
...
ctg_pos_t * col_pos(diff_ary_t *diff_cov, sdict_t *ctgs, ctg_pos_t *rmap)
{
	int i;
	int lb, ub;
	uint8_t *rblk = NULL;
	...
	for ( i = 0; i < ctgs->n_seq; ++i) {
		...
		fprintf(stderr, "%s\t%d\t%d\t%d\n", ctgs->seq[i].name, lb, ub, n_ctg_pos);
		for ( j = 0; j < diff_cov[i].n; ++j) { // too many leavel arrays //change later  
			if (diff_cov[i].dfs[j].diff > lb && diff_cov[i].dfs[j].diff < ub) {
			uint32_t s = diff_cov[i].dfs[j].s, e = diff_cov[i].dfs[j].e;
			pos_push(&d->ctg_pos[i], s << 1);
			pos_push(&d->ctg_pos[i], e << 1 | 1);
...
int aa_bionano(char *xmap_fn, char *rmap_fn, char *qmap_fn, char *key_fn, float min_conf, int min_cov, float min_cov_rat, int max_cov, char *out_dir) 
{
	...
	if (!rctg_p) {
   	...
   	uint32_t i;
	for ( i = 0; i < ctgs->n_seq; ++i) {
		pos_t * pt = &rctg_p->ctg_pos[i];
		uint32_t j;
		for ( j = 0; j < pt->n; ++j) {
			fprintf(stderr, "%s\t%u\t%u\n", ctgs->seq[i].name, j + 1, pt->p[j]);
		}
	}	
	int n_qry;
#ifdef VERBOSE
...
#endif
	ctg_pos_t *d = col_pos(dif_cov, ctgs, rctg_p);	
	//release memory here
#ifdef VERBOSE
...
#ifdef VERBOSE
	fprintf(stderr, "[M::%s] releasing memory resources\n", __func__);
#endif
	cov_ary_destroy(ca, ctgs->n_seq);
	sd_destroy(ctgs);
...
help:	
	...
	aa_bionano(xmap_fn, rmap_fn, qmap_fn, key_fn, min_conf, min_cov, min_cov_rat, max_cov, out_dir);
