//  Adapt from Hengli's code should add license here
//
//
//
//
...
#include "kvec.h"
#include "kseq.h"
KSEQ_INIT2(, gzFile, gzread, gzseek)
...
eg_bseq_file_t *eg_bseq_open(const char *fn)
{
	...
	if (f == 0) return 0;
	fp = (eg_bseq_file_t*)calloc(1, sizeof(eg_bseq_file_t));
	fp->fp = f;
...
static inline void kseq2bseq(kseq_t *ks, eg_bseq1_t *s, int with_qual)
{
	int i;
...
{
	int64_t size = 0;
	kvec_t(eg_bseq1_t) a = {0,0,0};
	kseq_t *ks = fp->ks;
	*n_ = 0;
	if (fp->s.seq) {
		kv_resize(eg_bseq1_t, a, 256);
		kv_push(eg_bseq1_t, a, fp->s);
		size = fp->s.l_seq;
		memset(&fp->s, 0, sizeof(eg_bseq1_t));
	}
	while (kseq_read(ks) >= 0) {
		eg_bseq1_t *s;
		assert(ks->seq.l <= INT32_MAX);
		if (a.m == 0) kv_resize(eg_bseq1_t, a, 256);
		kv_pushp(eg_bseq1_t, a, &s);
		...
		if (size >= chunk_size) {
			if (frag_mode && a.a[a.n-1].l_seq < CHECK_PAIR_THRES) {
				while (kseq_read(ks) >= 0) {
					...
					if (eg_qname_same(fp->s.name, a.a[a.n-1].name)) {
						kv_push(eg_bseq1_t, a, fp->s);
						memset(&fp->s, 0, sizeof(eg_bseq1_t));
					} else break;
				}
...
{
	return eg_bseq_read2(fp, chunk_size, with_qual, 0, n_);
}
...
int eg_bseq_destroy(eg_bseq1_t *egb, int n_bseq)
{
	if (egb) {
		int i;
		for ( i = 0; i < n_bseq; ++i) {
			if (egb[i].name) free(egb[i].name);
			if (egb[i].seq) free(egb[i].seq);
			if (egb[i].qual) free(egb[i].qual);
		}
		free(egb);
	}
	return 0;
